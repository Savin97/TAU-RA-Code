...
from functions.fetch_scores import download_reviewed_folder
from functions.per_composer_functions import rootdiff_bigram_weight_matrix, row_normalize_prog_weight_matrix
from functions.utilities import create_composer_file_lists, get_diff_categories_trimmed, load_tsv, make_csv, check_dirs
from functions.my_functions import (aggregate_prog_transition_unconditional, 
                          ...
                          build_composer_prog_trans_df)
from functions.per_piece_functions import (add_annotation_duration, add_bigram_weight, add_prog_weight, 
                                           add_root_diff, 
                                           add_root_progression_type_simple,
                                           add_root_progression_type_fine, 
                                           convert_frac_cols_to_float, 
                                           drop_unnecessary_columns, rootdiff_progweight_sum_table, uri_system_filter)
from functions.visualization import plot_progression_heatmap, save_heatmaps
...
def main():
    ...
    for composer, tsv_files in composer_dict.items():
        ...
        global_bigram_matrix_martin = pd.DataFrame(0.0, index=ALL_PROGRESSION_VALUES_MARTIN, columns=ALL_PROGRESSION_VALUES_MARTIN)
        ...
        for tsv in tsv_files:
            ...
            for root_diff, wsum in piece_sums.items():
                composer_root_diff_weight[ int(root_diff)] += float(wsum)

            piece_mat = rootdiff_bigram_weight_matrix(df)
            global_bigram_matrix += piece_mat  # if you initialize a global_bigram_mat DataFrame
            global_bigram_matrix_martin += piece_mat  
            # -----------------------------
        ...
        # after all pieces:
        uncond_weight_joint = unconditional_joint_probs(global_bigram_matrix)
        uncond_weight_joint_martin = unconditional_joint_probs(global_bigram_matrix_martin)
        make_csv(composer_rootdiff_weight_df, f"{composer}_rootdiff_progweight_sum")

        save_heatmaps(composer, cond_probs, uncond_probs, cond_probs_fine_uri, uncond_probs_fine_uri, cond_all_probs, uncond_all_probs, diff_cats_trim, uncond_weight_joint, uncond_weight_joint_martin)
        # Output to .csv
