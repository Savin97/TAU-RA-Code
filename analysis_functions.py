...
def classify_root_movement_fine(diff):
    """
...
def root_progression(df):
    ...
    # new categorization
    df["progression_fine"] = df["root_change"].apply(classify_root_movement_fine)
...
def get_fine_progression_matrices(composer: str, reviewed_tsv_files):
    ...
    for score in reviewed_tsv_files:
        ...
        if "fine_prog" not in dfp.columns:
            dfp["fine_prog"] = dfp["root_change"].map(classify_root_movement_fine)
...
# def get_root_change_new_division(composer: str, reviewed_tsv_files):
#     """
#     Aggregate (sum) root-change transition counts across many TSVs and plot both:
...
#     Uses plot_progression_heatmap for visualization.
#     """
#     diffs = list(FINE_PROGRESSION_LABELS)
#     global_counts = pd.DataFrame(0, index=diffs, columns=diffs, dtype=int)
...
#     plot_progression_heatmap(f"{composer}_DIFF_COND", cond, categories=ROOT_DIFF_VALUES)
#     plot_progression_heatmap(f"{composer}_DIFF_UNCOND", uncond, categories=ROOT_DIFF_VALUES)
...
# from config import PROGRESSION_CATEGORIES, ALL_PROG_LABELS, ROOT_DIFF_VALUES
# from visualization import plot_progression_heatmap
...
# # -------------------------------------------------------------------
# # 22/1/26 Root-change (diff value) transition matrix (requested 21x21)
# # -------------------------------------------------------------------
...
# def get_root_change_matrices(composer: str, reviewed_tsv_files, diffs=ROOT_DIFF_VALUES):
#     """
#     Aggregate (sum) root-change transition counts across many TSVs and plot both:
...
#     plot_progression_heatmap(f"{composer}_DIFF21_COND", cond)
#     plot_progression_heatmap(f"{composer}_DIFF21_UNCOND", uncond)
